...
class AntColony:
    ...
    def intitlialize(
        ...
        with open(f"MMAS_VNS.txt", "a") as file:
            file.write(
                f"instance: {self.instance} ,time_take: {self.end_time-self.start_time:.2f} sec, iterations: {self.iterations},"
        ...
        print(
            f"Best Distance Found: {(round(self.global_best_distance, 2))} | Mode: {self.mode} | Instance: {self.instance} | Time_taken: {self.end_time-self.start_time:.2f} sec | Iterations: {self.iterations}\n"
            f" Best Route Found: {self.global_best_tour}\n"
        )
    ...
    def lay_pheromone(self, solution, distance_travelled, factor: float = None) -> None:
        if factor is None:
        ...
        for id in range(1, self.maxiterations + 1):
            ...
            for _ in range(self.ant_count):
                ...
                if self.enable_search:
                    route_distance, solution, time, iterations = Local_Search.vns(solution)
    ...
    def minmax(self) -> None:
        ...
        for id in range(1, self.maxiterations + 1):
            ...
            if self.enable_search:
                iteration_best_distance, iteration_best_tour, time, iterations = Local_Search.vns(iteration_best_tour)
